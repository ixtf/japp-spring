...
public class UserController extends AbstractController {
	...
	@RequestMapping(value = AdminWeb.userList, method = RequestMethod.GET)
	public ModelAndView userList() throws Exception {
		return userList(0);
	}

	@RequestMapping(value = AdminWeb.userList + "/page/{pageNumber}", method = RequestMethod.GET)
	public ModelAndView userList(@PathVariable("pageNumber") Integer pageNumber)
		...
		return new ModelAndView(AdminWeb.userListView)
				.addObject("users", users.build())
				.addObject("pageNumber", pageNumber)
				.addObject("command", new UserSearchCommand());
	}

	@RequestMapping(value = AdminWeb.userSearch, method = RequestMethod.POST)
	public ModelAndView userSearch(
		...
		return new ModelAndView(AdminWeb.userListView).addObject("users",
				users.build()).addObject("command", command);
	}

	@RequestMapping(value = AdminWeb.userUpdate, method = RequestMethod.GET)
	public ModelAndView userUpdate() throws Exception {
		return new ModelAndView(AdminWeb.userUpdateView).addObject("command",
				new User());
	}

	@RequestMapping(value = AdminWeb.userList + "/{uuid}", method = RequestMethod.GET)
	...
	@RequestMapping(value = { AdminWeb.userUpdate,
			AdminWeb.userList + "/{uuid}" }, method = RequestMethod.POST)
	...
	@RequestMapping(value = AdminWeb.userBind + "/{uuid}", method = RequestMethod.GET)
	...
	@RequestMapping(value = { AdminWeb.userBind, AdminWeb.userBind + "/{uuid}" }, method = RequestMethod.POST)
